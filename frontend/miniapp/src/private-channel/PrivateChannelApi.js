...
export async function getPrivateChannelState({ signal, channelType = 'TELEGRAM' } = {}) {
  const channel = String(channelType).toUpperCase();
  return parse(await fetch(`/api/v1/private-channel/me?channel=${encodeURIComponent(channel)}`, { signal, headers: authHeaders() }), 'Не удалось загрузить подписку.');
}
export async function startPrivateChannelCheckout({ recurringEnabled, channelType = 'TELEGRAM' }) {
  const channel = String(channelType).toUpperCase();
  return parse(await fetch('/api/v1/private-channel/me/checkout', {
    ...
    body: JSON.stringify({ channelType: channel, recurringEnabled, recurringConsentVersion: recurringEnabled ? 'private-channel-recurring-v1' : undefined }),
