  ...
  return (
    <main className="terminal-shell">
      <header className="terminal-header">
        <BrandMark />
        <div className="terminal-point"><span className="online-dot" />{machineId ? `Автомат ${machineId} · серверная цена` : 'Автомат не определён'}</div>
      </header>
        ...
        <div className="terminal-layout">
          <section className="terminal-main">
            ...
            <div className="terminal-product">
              ...
              <div><span className="terminal-pill">Вкус дня</span><h2>{catalog.flavor.name.ru}</h2><p>Нежное ванильное мороженое, один сироп и один топпинг уже входят в стоимость.</p><div className="included-list"><span>✓ Стаканчик</span><span>✓ Сироп</span><span>✓ Топпинг</span></div></div>
            </div>
      ...
      {step === 1 && (
        <section className="payment-screen">
          ...
          <aside className="payment-order-card"><ProductArtwork syrupId={syrupId} toppingId={toppingId} /><h2>{catalog.product.name.ru}</h2><p>{syrup.name.ru} · {topping.name.ru}</p><strong>{payment.amount} ₽</strong></aside>
        </section>
      )}

      {step === 2 && (
        <section className="success-screen">
          <div className="success-check">✓</div><p className="terminal-kicker">{payment.paymentRequired ? 'Оплата подтверждена' : 'Подарок подтверждён'}</p><h1>{payment.fulfillment === 'machine' ? 'Начинаем готовить!' : 'Покажите код продавцу'}</h1><p>{payment.fulfillment === 'machine' ? 'Заказ передан автомату. Заберите десерт после сигнала готовности.' : 'Продавец уже получил уведомление об оплаченном заказе.'}</p>
          {(payment.giftAmount > 0 || payment.promotionDiscountAmount > 0) && <div className="terminal-saving">Вы сэкономили {payment.giftAmount + payment.promotionDiscountAmount} ₽</div>}
          <div className="sale-code"><span>Заказ</span><strong>{payment.orderId}</strong><span>Код выдачи</span><b>{payment.saleCode}</b></div><div className="fulfillment-status"><span>✓ Заказ подтверждён</span><span>{payment.fulfillment === 'machine' ? '● Команда выдачи отправлена автомату' : '● Продавец уведомлён и сверит код'}</span></div><button className="terminal-cta compact" type="button" onClick={restart}>Новый заказ</button>
        </section>
      )}
    </main>
  );
