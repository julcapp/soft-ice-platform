...
import{CatalogService}from'./services/catalogService.js';import{PriceService}from'./services/priceService.js';import'./styles.css';
const ICE='/media/ice/UT-ICE-Hero-001.png',OWNER='/media/brand/owner.jpg',POS='/media/payment/pos-terminal.png';
const TERMINAL={id:'TMS-001',city:'Томск',place:'Городской сад',detail:'главный вход',latitude:56.473333,longitude:84.954444,adminUrl:'https://admin.utimoshi.ru'};
const NONE_S={id:'none',name:'Без посыпки',price:0,available:true},NONE_T={id:'none',name:'Без топпинга',price:0,available:true};
const money=(v,c='RUB')=>PriceService.format(v,c);
const weatherInfo=code=>{if(code===0)return['☀️','Ясно'];if([1,2].includes(code))return['🌤️','Небольшая облачность'];if(code===3)return['☁️','Облачно'];if([45,48].includes(code))return['🌫️','Туман'];if([51,53,55,56,57].includes(code))return['🌦️','Морось'];if([61,63,65,66,67,80,81,82].includes(code))return['🌧️','Дождь'];if([71,73,75,77,85,86].includes(code))return['🌨️','Снег'];if([95,96,99].includes(code))return['⛈️','Гроза'];return['🌤️','Погода']};
function Top(){const[n,setN]=useState(new Date()),[w,setW]=useState({temp:null,code:null}),tap=useRef({count:0,last:0});useEffect(()=>{const i=setInterval(()=>setN(new Date()),30000);return()=>clearInterval(i)},[]);useEffect(()=>{let alive=true;const load=async()=>{try{const r=await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${TERMINAL.latitude}&longitude=${TERMINAL.longitude}&current=temperature_2m,weather_code&timezone=Asia%2FTomsk`);if(!r.ok)throw new Error();const j=await r.json();if(alive&&j.current)setW({temp:Math.round(j.current.temperature_2m),code:j.current.weather_code})}catch{}};load();const i=setInterval(load,600000);return()=>{alive=false;clearInterval(i)}},[]);const[icon,text]=weatherInfo(w.code);const adminTap=()=>{const now=Date.now();tap.current.count=now-tap.current.last<900?tap.current.count+1:1;tap.current.last=now;if(tap.current.count>=3){tap.current.count=0;window.location.href=TERMINAL.adminUrl}};return <header className="terminal-header"><div className="brand-left"><img src={OWNER}/><div><strong>У Тимоши</strong><small>Счастье в одном стаканчике!</small></div><i>♡</i></div><div className="weather-center"><div><span>{new Intl.DateTimeFormat('ru-RU',{day:'numeric',month:'long',timeZone:'Asia/Tomsk'}).format(n)}</span><b>{n.toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit',timeZone:'Asia/Tomsk'})}</b><span className="weather-value">{w.temp===null?'Погода —':`${icon} ${w.temp>0?'+':''}${w.temp}°`}</span></div><small>{w.temp===null?'Получаем данные погоды…':`${text}, ${TERMINAL.city}`}</small></div><button className="terminal-info" onClick={adminTap} aria-label="Информация об аппарате"><strong>Аппарат № {TERMINAL.id}</strong><small>{TERMINAL.place}, {TERMINAL.detail}</small></button></header>}
function Shell({children,back=true,onBack}){return <main className="approved"><Top/>{children}{back&&<button className="back" onClick={onBack}>←&nbsp; Назад</button>}</main>}
