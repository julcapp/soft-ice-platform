...
export function AdminNotificationBell() {
  ...
  return <div style={{ position: 'relative' }}>
    ...
    {open && <div className="card" role="dialog" aria-label="Центр уведомлений администратора" style={{ position: 'absolute', right: 0, top: 'calc(100% + 10px)', width: 'min(520px, calc(100vw - 24px))', maxHeight: '70vh', overflow: 'auto', zIndex: 30, padding: 16 }}>
      <div className="card-heading" style={{ alignItems: 'center' }}><div><h2>Уведомления</h2><p style={{ margin: '4px 0 0' }}>Операционные события платформы</p></div>{unread > 0 && <button type="button" className="text-button" onClick={readAll}>Прочитать все</button>}</div>
