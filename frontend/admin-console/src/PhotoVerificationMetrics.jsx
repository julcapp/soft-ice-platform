...
export function PhotoVerificationMetricsPage({ loadMetrics = getPhotoVerificationMetrics }) {
  ...
  useEffect(() => {
    ...
    loadMetrics({ signal: controller.signal, period })
  ...
  const rangeLabel = useMemo(() => {
    if (!state.data?.period) return '';
    const start = new Date(state.data.period.startAt).toLocaleDateString('ru-RU');
    const end = new Date(state.data.period.endAt).toLocaleDateString('ru-RU');
    return `${start} — ${end}`;
  }, [state.data]);
  ...
  const { totals, decisions, channels, trend = [] } = state.data;
  return <section className="card" aria-label="Метрики проверки фотографий">
    <div className="card-heading">
      <div>
        <h2>Операционные метрики</h2>
        <p style={{ margin: '6px 0 0' }}>Нагрузка, качество модерации и надёжность публикаций. {rangeLabel}</p>
      </div>
      <StatusBadge status={(totals.publicationIncomplete + totals.rewardPending) > 0 ? 'PENDING' : 'ACTIVE'} />
    </div>

    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }} aria-label="Период метрик">
      {PERIODS.map(([value, label]) => <button
        key={value}
        type="button"
    ...
    <div className="tables" style={{ marginTop: 16 }}>
      {channels.map((channel) => <div className="card" key={channel.channel}>
        ...
        <p>Опубликовано: <strong>{channel.published}</strong> / {channel.total} · Успех: <strong>{channel.successPercent}%</strong></p>
    ...
    <div className="card" style={{ marginTop: 16 }}>
      <div className="card-heading"><strong>Динамика решений по дням</strong><StatusBadge status="ACTIVE" /></div>
      {!trend.length && <p>За выбранный период решений пока нет.</p>}
      {!!trend.length && <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr><th align="left">Дата</th><th>Auto approve</th><th>Auto reject</th><th>Ручная</th></tr></thead>
          <tbody>{trend.map((row) => <tr key={row.day}>
            <td>{new Date(row.day).toLocaleDateString('ru-RU')}</td><td align="center">{row.autoApproved}</td><td align="center">{row.autoRejected}</td><td align="center">{row.manual}</td>
          </tr>)}</tbody>
        </table>
      </div>}
    </div>
