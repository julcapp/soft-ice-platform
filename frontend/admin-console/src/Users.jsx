...
function UserCard({ customerId, clients }) {
  ...
  return <div className="crm-workspace">
    ...
    <section className="crm-grid">
      ...
      <DataTable title="Рефералы пользователя" rows={invited.map((item) => ({ ...item, person: item.referredCustomer ? `${item.referredCustomer.name || 'Без имени'} · ${item.referredCustomer.phone || item.referredCustomer.email || item.referredCustomer.id}` : item.referredCustomerId || 'Ещё не зарегистрирован', active: item.firstPurchaseAt ? 'ACTIVE' : 'PENDING' }))} columns={[{ key: 'person', label: 'Кто приглашён' }, { key: 'referralCode', label: 'Код' }, { key: 'active', label: 'Активность', render: (value) => <StatusBadge status={value} /> }, { key: 'firstPurchaseAt', label: 'Первая покупка', render: dateTime }]} emptyTitle="Рефералов пока нет" />
