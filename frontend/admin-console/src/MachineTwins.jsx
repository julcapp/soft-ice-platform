...
import { getMachineTwin, listMachineTwins } from './api/machineTwinClient';
import { EmptyState, ErrorState, Skeleton, StatusBadge } from './components';
import { ComponentList, FaultTimeline, FreshnessIndicator, MachineTwinCard, MachineTwinDiagram, PredictionSummaryCard, SnapshotHistory, SourceStatusPanel, TwinSummaryPanel } from './machineTwinComponents';
...
export function MachineTwinDetail({ twin, section = 'overview' }) {
  const sections = {
    overview: <><FreshnessIndicator freshness={twin.freshness} /><TwinSummaryPanel twin={twin} /><div className="detail-grid"><Identity twin={twin} /><SourceStatusPanel sources={twin.sourceStatus} /></div><MachineTwinDiagram components={twin.components} /><PredictionSummaryCard prediction={twin.predictionSummary} /></>,
    components: <ComponentList components={twin.components} />,
    events: <FaultTimeline faults={twin.activeFaults} events={twin.events || []} />,
...
function DetailTabs({ machineId }) { const sections = { overview: 'Обзор', components: 'Компоненты', events: 'События', snapshots: 'Снимки', health: 'Состояние' }; return <nav className="detail-tabs" aria-label="Разделы цифрового двойника">{Object.entries(sections).map(([section, label]) => <a key={section} href={`#machine-twins/${machineId}/${section}`}>{label}</a>)}</nav>; }
export function MachineTwinsPage({ listClient = listMachineTwins, detailClient = getMachineTwin, route = 'machine-twins' }) {
  const [state, setState] = useState({ status: 'loading' });
  const [, machineId, section = 'overview'] = route.split('/');
  useEffect(() => {
    const controller = new AbortController();
    const work = machineId ? detailClient(machineId, { signal: controller.signal }) : listClient({ signal: controller.signal });
  ...
  }, [detailClient, listClient, machineId]);
