...
import { getPhotoReviewPreview, getPhotoReviewQueue, submitPhotoReviewDecision } from './api/photoVerificationClient';
...
export function PhotoModerationQueuePage({
  ...
  const refresh = async (signal) => {
    const rows = await loadQueue({ signal });
  ...
  useEffect(() => {
    ...
    loadPreview(selectedId, { signal: controller.signal })
      .then((blob) => {
    ...
    <section className="card">
      ...
      {!!state.rows.length && <div style={{ display: 'grid', gridTemplateColumns: 'minmax(260px, 0.8fr) minmax(320px, 1.4fr)', gap: 18 }}>
        <div style={{ display: 'grid', gap: 10, alignContent: 'start' }}>
          {state.rows.map((row) => <button
            key={row.photoChallengeId}
            type="button"
            onClick={() => { setSelectedId(row.photoChallengeId); setReason(''); setNotice(''); }}
            className="card"
        ...
        {selected && <article className="card" style={{ display: 'grid', gap: 16 }}>
          ...
          <details>
            <summary>Технические и AI-сигналы</summary>
            <pre style={{ whiteSpace: 'pre-wrap', overflowWrap: 'anywhere' }}>{JSON.stringify({
              checks: selected.checks || {},
              metadata: selected.metadataResult || {},
              antifraud: selected.antifraudResult || {},
          ...
          <label>
            <strong>Причина / комментарий модератора</strong>
            <textarea value={reason} onChange={(event) => setReason(event.target.value)} rows={4} placeholder="Обязательно для отклонения и дополнительной проверки" style={{ width: '100%', marginTop: 8, padding: 10 }} />
          </label>
