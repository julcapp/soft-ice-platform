...
export function UserPaymentProfile({ customerId, client = getCustomerPaymentProfile, refundClient = createCustomerRefund }) {
  ...
  const economicsRows = (data.paymentHistory || []).filter((row) => row.economics).map((row) => ({ id: `economics:${row.id}`, occurredAt: row.occurredAt, linkedEntity: row.linkedEntity, paymentMethodType: row.paymentMethodType, ...row.economics }));
  return <section className="card" aria-label="Платёжный профиль пользователя">
    <div className="card-heading">
      <div><h2>Платёжный профиль</h2><p style={{ margin: '6px 0 0' }}>Способы оплаты, история платежей, экономика платёжного провайдера, чеки, возвраты, рекуррентные подписки и доказательства согласия. Бонусы не смешиваются с рублями.</p></div>
    ...
    <DataTable title="Экономика платежей" rows={economicsRows} columns={[
