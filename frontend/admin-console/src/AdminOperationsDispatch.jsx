...
export function AdminOperationsDispatch({ compact = false }) {
  ...
  return <section className="card" aria-label="Операционная диспетчерская" style={compact ? { marginTop: 12 } : undefined}>
    <div className="card-heading"><div><h2>Операционная диспетчерская</h2><p style={{ margin: '4px 0 0' }}>Рабочая очередь поверх исходных инцидентов. Прочтение не закрывает проблему.</p></div>{data && <StatusBadge status={data.summary.critical ? 'CRITICAL' : data.summary.open || data.summary.inProgress ? 'WARNING' : 'CLEAR'} />}</div>
    ...
    {data && <div className="statistics" style={{ marginBottom: 14 }}><div><strong>{data.summary.total}</strong><small> всего</small></div><div><strong>{data.summary.critical}</strong><small> критичных</small></div><div><strong>{data.summary.open}</strong><small> открыто</small></div><div><strong>{data.summary.inProgress}</strong><small> в работе</small></div><div><strong>{data.summary.resolved}</strong><small> решено</small></div></div>}
    ...
    {data?.items?.map((item) => {
      ...
      return <article key={item.key} style={{ borderTop: '1px solid var(--border-color, #ddd)', padding: '14px 0' }}>
        ...
        {history?.key === item.key && <div style={{ marginTop: 12, padding: 10, background: 'rgba(0,0,0,.03)' }}><strong>История обработки</strong>{history.events?.length ? <ul>{history.events.map((event) => <li key={event.id}>{new Date(event.createdAt).toLocaleString('ru-RU')} · {event.actorSubject} · {event.eventType}{event.fromStatus || event.toStatus ? ` · ${event.fromStatus || '—'} → ${event.toStatus || '—'}` : ''}{event.assigneeSubject ? ` · ответственный: ${event.assigneeSubject}` : ''}{event.comment ? ` · ${event.comment}` : ''}</li>)}</ul> : <p>Действий пока нет.</p>}</div>}
      </article>;
    })}
  </section>;
}
