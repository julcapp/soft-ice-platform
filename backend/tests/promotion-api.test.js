...
test('MARKETER can create Promotion Engine DRAFT and server owns createdBy', async () => {
  ...
  const promotionService = {
    createDraft: async (input) => { captured = input; return { id: 'promo-1', status: 'DRAFT', ...input }; },
  };
  await withServer(promotionService, async (server) => {
    const response = await request(server, {
      method: 'POST', path: '/api/v1/admin/promotions', headers: adminHeaders('MARKETER'),
      body: { code: 'HAPPY_HOUR', name: 'Час выгоды', createdBy: 'spoofed', version: { version: 1 } },
    });
    assert.equal(response.status, 201);
    assert.equal(captured.createdBy, 'admin-1');
...
test('OBSERVER cannot create promotion DRAFT', async () => {
  await withServer({ createDraft: async () => ({}) }, async (server) => {
    const response = await request(server, {
      method: 'POST', path: '/api/v1/admin/promotions', headers: adminHeaders('OBSERVER'), body: { code: 'X' },
    });
    assert.equal(response.status, 403);
    assert.equal(response.body.error.code, 'PROMOTION_PERMISSION_DENIED');
  });
});
...
test('MANAGER can validate DRAFT', async () => {
  let actorId;
  const promotionService = {
    validateDraft: async (input) => { actorId = input.actorId; return { campaignId: input.campaignId, status: 'READY', validation: { valid: true } }; },
  };
  await withServer(promotionService, async (server) => {
    const response = await request(server, {
      method: 'POST', path: '/api/v1/admin/promotions/promo-1/validate', headers: adminHeaders('MANAGER'),
    });
...
test('OBSERVER can read promotion campaign', async () => {
  ...
  await withServer(promotionService, async (server) => {
    ...
    assert.equal(response.status, 200);
    assert.equal(response.body.data.id, 'promo-1');
  });
});
