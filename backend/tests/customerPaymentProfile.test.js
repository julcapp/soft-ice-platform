...
test('payment profile combines history, receipts and refunds without exposing full provider secrets', async () => {
  ...
  const prisma = {
    ...
    $queryRawUnsafe: async (sql) => {
      ...
      if (sql.includes('FROM "PaymentProfileRefund"')) return [{ id: 'rf1', paymentSourceType: 'PAYMENT', paymentSourceId: 'p1', orderId: 'o1', subscriptionId: null, provider: 'YOOKASSA', providerRefundId: 'refund_123456789', status: 'SUCCEEDED', amountRub: 50, currency: 'RUB', reason: 'Частичный возврат', requestedAt: new Date('2026-08-22T10:00:00Z'), succeededAt: new Date('2026-08-22T10:01:00Z'), failedAt: null, createdAt: new Date('2026-08-22T10:00:00Z') }];
      return [];
    },
  };
