...
const { PhotoPublishingOrchestrator, PUBLICATION_STATUSES } = require('../src/modules/photo_verification/PhotoPublishingOrchestrator');

function repositoryDouble(existing = {}) {
  return {
    attempts: [], events: [], existing,
    async getPublicationAttempt(photoChallengeId, channel) { return this.existing[channel] || null; },
...
test('publishes to all configured required channels independently', async () => {
  ...
  const publisher = { async publish({ targetId }) { return { externalPublicationId: `post-${targetId}`, publicationUrl: `https://example.test/${targetId}` }; } };
  const orchestrator = new PhotoPublishingOrchestrator({
    repository, publishers: { VK: publisher, TELEGRAM: publisher, MAX: publisher },
    targets: {
  ...
  const result = await orchestrator.publishAll({ photoChallengeId: 'challenge-1', media: { objectKey: 'ugc/photo-1.jpg' }, caption: 'test', correlationId: 'corr-1' });
...
test('retry skips channels already published and preserves their URL', async () => {
  const repository = repositoryDouble({ VK: { status: 'published', externalPublicationId: 'vk-old', publicationUrl: 'https://vk.test/old' } });
  let vkCalls = 0;
  let telegramCalls = 0;
  const orchestrator = new PhotoPublishingOrchestrator({
    repository,
    publishers: {
      VK: { async publish() { vkCalls += 1; return { externalPublicationId: 'vk-new' }; } },
      TELEGRAM: { async publish() { telegramCalls += 1; return { externalPublicationId: 'tg-new' }; } },
    },
    targets: {
      VK: { channel: 'VK', targetId: 'vk', required: true },
      TELEGRAM: { channel: 'TELEGRAM', targetId: 'tg', required: true },
    },
  });
  const result = await orchestrator.publishAll({ photoChallengeId: 'challenge-retry', media: { objectKey: 'ugc/retry.jpg' } });
  assert.equal(result.allRequiredPublished, true);
  assert.equal(vkCalls, 0);
  assert.equal(telegramCalls, 1);
  assert.equal(result.results.find((item) => item.channel === 'VK').skipped, true);
  assert.equal(result.results.find((item) => item.channel === 'VK').publicationUrl, 'https://vk.test/old');
});
...
test('missing Telegram/MAX configuration does not fake success', async () => {
  ...
  const orchestrator = new PhotoPublishingOrchestrator({
    repository,
    publishers: { VK: { async publish() { return { externalPublicationId: 'vk-1' }; } } },
    targets: {
      VK: { channel: 'VK', targetId: 'club239119350', required: true },
  ...
  const result = await orchestrator.publishAll({ photoChallengeId: 'challenge-2', media: { objectKey: 'ugc/photo-2.jpg' } });
...
test('one channel failure does not roll back successful channels', async () => {
  ...
  const orchestrator = new PhotoPublishingOrchestrator({
    repository, publishers: { VK: successPublisher, TELEGRAM: failedPublisher, MAX: successPublisher },
    targets: {
      VK: { channel: 'VK', targetId: 'club239119350', required: true },
      TELEGRAM: { channel: 'TELEGRAM', targetId: 'tg-channel', required: true },
      MAX: { channel: 'MAX', targetId: 'max-channel', required: true },
