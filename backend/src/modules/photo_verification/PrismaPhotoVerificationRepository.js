...
class PrismaPhotoVerificationRepository {
  ...
  async listManualReviewQueue({ limit = 50 } = {}) {
    return this.prisma.$queryRaw`
      SELECT
        pc."id" AS "photoChallengeId", pc."customerId", pc."orderId", pc."photoFilePath" AS "storageKey",
        pc."status" AS "challengeStatus", pc."createdAt", pc."deadlineAt",
        c."name" AS "customerName", c."phone" AS "customerPhone",
        vr."decision", vr."confidence", vr."fraudScore", vr."reasonCode", vr."checks",
        vr."metadataResult", vr."antifraudResult", vr."provider", vr."model", vr."processedAt"
      FROM "PhotoChallenge" pc
      JOIN "Customer" c ON c."id" = pc."customerId"
      LEFT JOIN LATERAL (
        SELECT * FROM "PhotoVerificationResult" v
        WHERE v."photoChallengeId" = pc."id"
  ...
  async getManualReviewItem(photoChallengeId) {
  ...
  async findActiveCaptureChallenge({ photoChallengeId, customerId, now = new Date() }) {
    const rows = await this.prisma.$queryRaw`
      ...
      ORDER BY issued."createdAt" DESC LIMIT 1
    `;
    return rows[0] || null;
  }

  async consumeCaptureChallenge({ photoChallengeId, customerId, issuedEventId, correlationId, reason = 'submitted' }) {
    return this.recordEvent({ photoChallengeId, eventType: 'capture_challenge_consumed', eventSource: 'photo_capture_challenge_service', correlationId,
  ...
  async upsertFingerprint(input) {
    ...
    await this.prisma.$executeRaw`
      ...
      ON CONFLICT ("photoChallengeId") DO UPDATE SET "sha256" = EXCLUDED."sha256", "pHash" = EXCLUDED."pHash", "dHash" = EXCLUDED."dHash"
    `;
    return rowId;
  }

  async findFingerprintCandidates({ photoChallengeId, sha256, limit = 250 }) {
    return this.prisma.$queryRaw`SELECT "photoChallengeId", "sha256", "pHash", "dHash" FROM "PhotoFingerprint"
      WHERE "photoChallengeId" <> ${photoChallengeId}
      ORDER BY CASE WHEN "sha256" = ${sha256} THEN 0 ELSE 1 END, "createdAt" DESC LIMIT ${limit}`;
  }

  async recordPublication(input) { return this.upsertPublicationAttempt(input); }
  ...
  async upsertPublicationAttempt(input) {
    ...
    await this.prisma.$executeRaw`
      INSERT INTO "PhotoPublication" ("id", "photoChallengeId", "channel", "targetId", "status", "externalPublicationId",
        "publicationUrl", "publishedAt", "confirmedAt", "errorCode", "errorMessage", "attemptCount", "lastAttemptAt", "idempotencyKey")
      VALUES (${rowId}, ${input.photoChallengeId}, ${input.channel}, ${input.targetId || null}, ${input.status || 'pending'},
        ${input.externalPublicationId || null}, ${input.publicationUrl || null}, ${input.publishedAt || null}, ${input.confirmedAt || null},
        ${input.errorCode || null}, ${input.errorMessage || null}, 1, ${lastAttemptAt}, ${idempotencyKey})
      ON CONFLICT ("photoChallengeId", "channel") DO UPDATE SET
        "targetId" = EXCLUDED."targetId", "status" = EXCLUDED."status",
        ...
        "attemptCount" = "PhotoPublication"."attemptCount" + 1, "lastAttemptAt" = EXCLUDED."lastAttemptAt",
        "idempotencyKey" = EXCLUDED."idempotencyKey", "updatedAt" = CURRENT_TIMESTAMP
    `;
    return rowId;
  }

  async getSettings(scopeKey = 'default') {
    const rows = await this.prisma.$queryRaw`SELECT * FROM "PhotoVerificationSettings" WHERE "scopeKey" = ${scopeKey} LIMIT 1`;
    return rows[0] || null;
  }
  ...
  async upsertSettings(input) {
    ...
    const next = {
      scopeKey: input.scopeKey || 'default', enabled: input.enabled ?? current?.enabled ?? false,
      mode: input.mode ?? current?.mode ?? 'manual_only', publishingEnabled: input.publishingEnabled ?? current?.publishingEnabled ?? false,
      ...
      rewardBonusUnits: input.rewardBonusUnits !== undefined ? input.rewardBonusUnits : (current?.rewardBonusUnits ?? null),
      retentionPolicy: input.retentionPolicy ?? current?.retentionPolicy ?? 'delete_after_publication',
      provider: input.provider ?? current?.provider ?? null, model: input.model ?? current?.model ?? null, updatedBy: input.updatedBy ?? null,
    };
    await this.prisma.$executeRaw`
      INSERT INTO "PhotoVerificationSettings" ("scopeKey", "enabled", "mode", "publishingEnabled", "requiredChannels",
        "approvalThreshold", "rejectionThreshold", "maxFraudScore", "duplicateChecksEnabled", "metadataChecksEnabled",
        "challengeCodeEnabled", "rewardBonusUnits", "retentionPolicy", "provider", "model", "updatedBy")
      VALUES (${next.scopeKey}, ${next.enabled}, ${next.mode}, ${next.publishingEnabled}, ${JSON.stringify(next.requiredChannels)}::jsonb,
        ${next.approvalThreshold}, ${next.rejectionThreshold}, ${next.maxFraudScore}, ${next.duplicateChecksEnabled}, ${next.metadataChecksEnabled},
        ${next.challengeCodeEnabled}, ${next.rewardBonusUnits}, ${next.retentionPolicy}, ${next.provider}, ${next.model}, ${next.updatedBy})
      ON CONFLICT ("scopeKey") DO UPDATE SET
        "enabled" = EXCLUDED."enabled", "mode" = EXCLUDED."mode", "publishingEnabled" = EXCLUDED."publishingEnabled",
        "requiredChannels" = EXCLUDED."requiredChannels", "approvalThreshold" = EXCLUDED."approvalThreshold",
        "rejectionThreshold" = EXCLUDED."rejectionThreshold", "maxFraudScore" = EXCLUDED."maxFraudScore",
        "duplicateChecksEnabled" = EXCLUDED."duplicateChecksEnabled", "metadataChecksEnabled" = EXCLUDED."metadataChecksEnabled",
        "challengeCodeEnabled" = EXCLUDED."challengeCodeEnabled", "rewardBonusUnits" = EXCLUDED."rewardBonusUnits",
        "retentionPolicy" = EXCLUDED."retentionPolicy", "provider" = EXCLUDED."provider", "model" = EXCLUDED."model",
  ...
  async listCustomerPhotoHistory(customerId) {
    return this.prisma.$queryRaw`
      SELECT pc."id" AS "photoChallengeId", pc."createdAt" AS "createdAt", pc."status" AS "moderationStatus",
        COALESCE(jsonb_agg(jsonb_build_object('channel', pp."channel", 'status', pp."status", 'publicationUrl', pp."publicationUrl", 'publishedAt', pp."publishedAt"))
          FILTER (WHERE pp."id" IS NOT NULL), '[]'::jsonb) AS publications,
      ...
      GROUP BY pc."id", pc."createdAt", pc."status" ORDER BY pc."createdAt" DESC
    `;
  }
  ...
  async markSourceDeletion(input) {
    const rowId = input.id || id();
    await this.prisma.$executeRaw`
      INSERT INTO "PhotoSourceDeletion" ("id", "photoChallengeId", "storageKey", "status", "deleteReason", "requestedAt", "deletedAt", "errorMessage")
      VALUES (${rowId}, ${input.photoChallengeId}, ${input.storageKey}, ${input.status || 'pending'}, ${input.deleteReason || 'publication_confirmed'},
        ${input.requestedAt || null}, ${input.deletedAt || null}, ${input.errorMessage || null})
      ON CONFLICT ("photoChallengeId") DO UPDATE SET "storageKey" = EXCLUDED."storageKey", "status" = EXCLUDED."status",
        "deleteReason" = EXCLUDED."deleteReason", "requestedAt" = EXCLUDED."requestedAt", "deletedAt" = EXCLUDED."deletedAt",
        "errorMessage" = EXCLUDED."errorMessage", "updatedAt" = CURRENT_TIMESTAMP
    `;
    return rowId;
  }
}
