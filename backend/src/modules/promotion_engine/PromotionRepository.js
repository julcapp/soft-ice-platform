...
class PromotionRepository {
  ...
  async requestApproval({ campaignId, promotionVersionId, approvalPolicy, requestedBy, reason = null, metadata = undefined }) { return this.prisma.$transaction(async (tx) => { await tx.promotionApproval.updateMany({ where: { promotionVersionId, status: 'PENDING' }, data: { status: 'CANCELLED', decidedAt: new Date(), reason: 'Superseded by a new approval request.' } }); const request = await tx.promotionApproval.create({ data: { campaignId, promotionVersionId, approvalPolicy, status: 'PENDING', requestedBy, reason, metadata } }); await tx.promotionEvent.create({ data: { campaignId, promotionVersionId, eventType: 'APPROVAL_REQUESTED', actorType: 'ADMIN_USER', actorId: requestedBy, newValue: { approvalId: request.id, approvalPolicy }, reason, metadata } }); return request; }); }
  ...
  async recordApprovalDecision({ campaignId, promotionVersionId, approvalPolicy, requestedBy, decidedBy, status, reason = null, metadata = undefined }) { return this.prisma.$transaction(async (tx) => { const decision = await tx.promotionApproval.create({ data: { campaignId, promotionVersionId, approvalPolicy, status, requestedBy, decidedBy, decidedAt: new Date(), reason, metadata } }); await tx.promotionEvent.create({ data: { campaignId, promotionVersionId, eventType: status === 'APPROVED' ? 'APPROVAL_GRANTED' : 'APPROVAL_REJECTED', actorType: 'ADMIN_USER', actorId: decidedBy, newValue: { approvalId: decision.id, status, approvalPolicy }, reason, metadata } }); if (status === 'REJECTED') await tx.promotionApproval.updateMany({ where: { promotionVersionId, status: 'PENDING' }, data: { status: 'REJECTED', decidedBy, decidedAt: new Date(), reason } }); return decision; }); }
  ...
  async getUsageSummary(promotionVersionId) { const aggregate = await this.prisma.promotionApplication.aggregate({ where: { promotionVersionId }, _count: { _all: true }, _sum: { discountAmount: true, baseAmount: true, finalAmount: true }, _min: { finalAmount: true } }); return { applications: aggregate._count?._all || 0, discountAmount: Number(aggregate._sum?.discountAmount || 0), baseAmount: Number(aggregate._sum?.baseAmount || 0), finalAmount: Number(aggregate._sum?.finalAmount || 0), minimumObservedFinalAmount: aggregate._min?.finalAmount == null ? null : Number(aggregate._min.finalAmount) }; }
  async recordEvent({ campaignId, promotionVersionId = null, eventType, actorType = 'SYSTEM', actorId = null, reason = null, metadata = undefined, oldValue = undefined, newValue = undefined }) { return this.prisma.promotionEvent.create({ data: { campaignId, promotionVersionId, eventType, actorType, actorId, reason, metadata, oldValue, newValue } }); }
}
