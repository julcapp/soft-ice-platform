...
class PaymentOperationsService {
  ...
  async createRefund({ customerId, sourceType, sourcePaymentId, amountRub, reason, actorId = 'admin', idempotencyKey }) {
    ...
    const returnedRows = await this.prisma.$queryRawUnsafe(`SELECT COALESCE(SUM("amountRub"),0)::float8 AS total FROM "PaymentRefund" WHERE "customerId"=$1 AND "sourceType"=$2 AND "sourcePaymentId"=$3 AND "status" IN ('PENDING','SUCCEEDED')`, customerId, source.sourceType, source.sourcePaymentId);
    ...
    try {
      ...
      const status = normalizeRefundStatus(providerRefund.status); const completedAt = status === 'SUCCEEDED' ? now : null;
      await this.prisma.$executeRawUnsafe(`UPDATE "PaymentRefund" SET "providerRefundId"=$2,"status"=$3,"completedAt"=$4,"updatedAt"=$5 WHERE "id"=$1`, id, providerRefund.id || null, status, completedAt, now);
      if (providerRefund.receipt_registration) await this.recordReceipt({ customerId, sourceType: source.sourceType, sourcePaymentId: source.sourcePaymentId, orderId: source.orderId, subscriptionId: source.subscriptionId, provider: source.provider, receiptType: 'REFUND', amountRub: amount, status: String(providerRefund.receipt_registration).toUpperCase(), issuedAt: completedAt });
  ...
  async handleRefundSucceeded(providerRefundId) {
    ...
    await this.prisma.$executeRawUnsafe(`UPDATE "PaymentRefund" SET "status"='SUCCEEDED',"completedAt"=COALESCE("completedAt",$2),"updatedAt"=$2 WHERE "id"=$1`, row.id, now);
    if (authoritative.receipt_registration) await this.recordReceipt({ customerId: row.customerId, sourceType: row.sourceType, sourcePaymentId: row.sourcePaymentId, orderId: row.orderId, subscriptionId: row.subscriptionId, provider: row.provider, receiptType: 'REFUND', amountRub: Number(row.amountRub || authoritative.amount?.value || 0), status: String(authoritative.receipt_registration).toUpperCase(), issuedAt: now });
    ...
    return { handled: true, refundId: row.id, status: 'SUCCEEDED', completedAt: now };
  }
