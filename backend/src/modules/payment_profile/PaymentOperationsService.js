...
class PaymentOperationsService {
  ...
  async createRefund({ customerId, sourceType, sourcePaymentId, amountRub, reason, actorId = 'admin', idempotencyKey }) {
    ...
    const returnedRows = await this.prisma.$queryRawUnsafe(`SELECT COALESCE(SUM("amountRub"),0)::float8 AS total FROM "PaymentProfileRefund" WHERE "customerId"=$1 AND "paymentSourceType"=$2 AND "paymentSourceId"=$3 AND "status" IN ('PENDING','SUCCEEDED')`, customerId, source.sourceType, source.sourcePaymentId);
    ...
    try {
      ...
      await this.prisma.$executeRawUnsafe(`UPDATE "PaymentProfileRefund" SET "providerRefundId"=$2,"status"=$3,"succeededAt"=$4,"failedAt"=$5,"updatedAt"=$6 WHERE "id"=$1`, id, providerRefund.id || null, status, succeededAt, failedAt, now);
      ...
      await this.prisma.$executeRawUnsafe(`UPDATE "PaymentProfileRefund" SET "status"='FAILED',"failureReason"=$2,"failedAt"=$3,"updatedAt"=$3 WHERE "id"=$1`, id, String(error.code || error.message || 'UNKNOWN').slice(0, 500), now);
  ...
  async handleRefundSucceeded(providerRefundId) {
    ...
    const rows = await this.prisma.$queryRawUnsafe('SELECT * FROM "PaymentProfileRefund" WHERE "providerRefundId"=$1 LIMIT 1', providerRefundId);
    const row = rows[0]; if (!row) return { handled: false, reason: 'REFUND_NOT_TRACKED' };
    const now = this.clock();
    await this.prisma.$executeRawUnsafe(`UPDATE "PaymentProfileRefund" SET "status"='SUCCEEDED',"succeededAt"=COALESCE("succeededAt",$2),"updatedAt"=$2 WHERE "id"=$1`, row.id, now);
  ...
  async stats({ from, toExclusive }) {
    const [refunds, receipts] = await Promise.all([
      this.prisma.$queryRawUnsafe(`SELECT COUNT(*)::int AS count, COALESCE(SUM("amountRub"),0)::float8 AS amount FROM "PaymentProfileRefund" WHERE "status"='SUCCEEDED' AND "succeededAt">=$1 AND "succeededAt"<$2`, from, toExclusive),
