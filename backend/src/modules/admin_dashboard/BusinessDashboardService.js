...
class BusinessDashboardService {
  ...
  async getDashboard(securityContext, query = {}) {
    ...
    const [
      ...
      this.prisma.clubTopup.findMany({
        where: { paidAt: { gte: range.from, lt: range.toExclusive } },
        select: { amountRub: true, paidAt: true, customerId: true },
      }),
      this.prisma.order.findMany({
        where: { paidAt: { gte: range.from, lt: range.toExclusive } },
        select: { id: true, customerId: true, status: true, amount: true, amountPaidRub: true, paidAt: true, activePickupCodeHash: true },
      }),
      this.prisma.referral.findMany({
        select: { referrerCustomerId: true, referredCustomerId: true, status: true, firstPurchaseAt: true, referralCode: true },
      }),
      this.prisma.customerChannelSubscription.findMany({
        where: { isSubscribed: true },
        select: { channelType: true, targetType: true, targetExternalId: true, customerId: true, subscribedAt: true },
      }),
    ]);
    ...
    return {
      ...
      club: {
        ...
        joinedInPeriod: newClubMembers,
        paidTopupsInPeriod: paidTopups.length,
        topupAmountRubInPeriod: sum(paidTopups, (item) => item.amountRub),
      },
      referrals: {
        linksDistributed: null,
        linksDistributedStatus: 'BLOCKED',
        linksDistributedReason: 'REFERRAL_SHARE_EVENT_NOT_TRACKED',
      ...
      privateChannel: {
        subscribers: null,
        paidAmountRubInPeriod: null,
        forecastNext30DaysRub: null,
        status: 'BLOCKED',
        reason: 'PRIVATE_CHANNEL_BILLING_SOURCE_NOT_IMPLEMENTED',
      },
      sourceReadiness: {
        users: 'READY', club: 'READY', referralsAccepted: 'READY', referralShares: 'BLOCKED',
  ...
  async #topReferrer(activeReferrals, range) {
    ...
    return {
      customerId,
      displayName: customer?.name || (customer?.telegramUsername ? `@${customer.telegramUsername}` : maskPhone(customer?.phone)) || customerId,
...
function isPrivateTarget(item) {
  const targetType = String(item.targetType || '').toUpperCase();
  return targetType.includes('PRIVATE') || targetType.includes('PAID');
}

function buildDailySeries(range, orders) {
  const days = new Map();
  for (let cursor = new Date(range.from); cursor < range.toExclusive; cursor.setUTCDate(cursor.getUTCDate() + 1)) {
    days.set(isoDay(cursor), { date: isoDay(cursor), purchases: 0, revenueRub: 0 });
  }
  for (const order of orders) {
    const key = isoDay(order.paidAt);
    const row = days.get(key);
    if (!row) continue;
    row.purchases += 1;
    row.revenueRub += Number(order.amountPaidRub ?? order.amount ?? 0);
  }
  return [...days.values()].map((row) => ({ ...row, revenueRub: round(row.revenueRub, 2) }));
}

function resolveRange(query, now) {
  const fallbackTo = startUtcDay(now);
  const toInclusive = query.to ? parseDay(query.to, 'to') : fallbackTo;
  const from = query.from ? parseDay(query.from, 'from') : addDays(toInclusive, -29);
  if (from > toInclusive) throw validation('BUSINESS_DASHBOARD_PERIOD_INVALID', '`from` must not be after `to`.');
  const days = Math.floor((toInclusive - from) / 86400000) + 1;
  if (days > 366) throw validation('BUSINESS_DASHBOARD_PERIOD_TOO_LARGE', 'Maximum reporting period is 366 days.');
  return { from, toInclusive, toExclusive: addDays(toInclusive, 1), days };
}
function parseDay(value, field) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(value || ''))) throw validation('BUSINESS_DASHBOARD_DATE_INVALID', `Invalid ${field} date.`);
  const date = new Date(`${value}T00:00:00.000Z`);
  if (Number.isNaN(date.getTime())) throw validation('BUSINESS_DASHBOARD_DATE_INVALID', `Invalid ${field} date.`);
  return date;
}
...
function assertAdmin(securityContext) {
  ...
  if (!roles.some((role) => ADMIN_ROLES.has(role))) throw new ApiError({ statusCode: 403, code: 'ADMIN_PERMISSION_DENIED', message: 'Business dashboard access is not permitted.', source: 'platform_service' });
}

module.exports = { BusinessDashboardService, resolveRange, buildDailySeries };
