...
class CustomerProfileCommunicationService {
  constructor({ prisma, clock = () => new Date(), tokenTtlMs = EMAIL_VERIFICATION_TTL_MS }) {
  ...
  async getProfileState(customerId) {
    const customer = await this.prisma.customer.findUnique({
      where: { id: customerId },
      select: { id: true, name: true, phone: true, birthday: true, email: true },
    });
    ...
    return {
      customer,
      completion: { birthdayMissing: !customer.birthday, emailMissing: !customer.email },
      emailVerification: verification,
      marketingConsent: consent,
      unreadNotifications: Number(unreadRows?.[0]?.count || 0),
      rules: { version: MARKETING_RULES_VERSION, url: MARKETING_RULES_URL },
    };
  }

  async ensureProfilePrompts(customerId) {
    const state = await this.getProfileState(customerId);
    if (state.completion.birthdayMissing) {
      await this.#ensureNotification(customerId, {
        type: 'PROFILE_BIRTHDAY_MISSING', title: 'Добавьте дату рождения',
        body: 'Укажите дату рождения в профиле — в день рождения мы сможем подготовить для вас подарок.',
        actionType: 'OPEN_PROFILE_BIRTHDAY', significant: false,
      });
    }
    if (state.completion.emailMissing) {
      await this.#ensureNotification(customerId, {
        type: 'PROFILE_EMAIL_MISSING', title: 'Добавьте электронную почту',
        body: 'Укажите электронную почту для электронных чеков и сервисных сообщений. Рассылки будут отправляться только при отдельном согласии.',
        actionType: 'OPEN_PROFILE_EMAIL', significant: true,
      });
    }
  ...
  async updateProfile(customerId, input = {}) {
    ...
    if (Object.hasOwn(input, 'birthday')) {
      if (!/^\d{4}-\d{2}-\d{2}$/.test(String(input.birthday || ''))) throw validation('CUSTOMER_BIRTHDAY_INVALID', 'birthday must use YYYY-MM-DD.');
      const birthday = new Date(`${input.birthday}T00:00:00.000Z`);
      if (Number.isNaN(birthday.getTime()) || birthday > this.clock()) throw validation('CUSTOMER_BIRTHDAY_INVALID', 'birthday is invalid.');
      data.birthday = birthday;
    }
    if (Object.hasOwn(input, 'email')) {
      const email = normalizeEmail(input.email);
      if (!email) throw validation('CUSTOMER_EMAIL_INVALID', 'email is invalid.');
      data.email = email;
    }
    if (!Object.keys(data).length) throw validation('CUSTOMER_PROFILE_PATCH_EMPTY', 'No supported profile fields were provided.');
    const customer = await this.prisma.customer.update({ where: { id: customerId }, data, select: { id: true, birthday: true, email: true } });
    return customer;
  }
  ...
  async requestEmailVerification(customerId, emailInput) {
    ...
    await this.#ensureNotification(customerId, {
      type: 'EMAIL_VERIFICATION_PENDING', title: 'Подтвердите электронную почту',
      body: 'Мы отправили ссылку подтверждения. Ссылка действует 24 часа.', actionType: 'VERIFY_EMAIL', significant: true,
    }, true);
  ...
  async confirmEmailVerification(token) {
    ...
    const now = this.clock();
    const rows = await this.prisma.$queryRaw`SELECT "id","customerId","email","status","expiresAt" FROM "CustomerEmailVerification" WHERE "tokenHash" = ${tokenHash} LIMIT 1`;
    const row = rows?.[0];
    if (!row) throw validation('EMAIL_VERIFICATION_TOKEN_INVALID', 'Verification token is invalid.');
    if (row.status !== 'PENDING') throw validation('EMAIL_VERIFICATION_TOKEN_USED', 'Verification token is no longer active.');
    if (new Date(row.expiresAt) <= now) {
      await this.prisma.$executeRaw`UPDATE "CustomerEmailVerification" SET "status"='EXPIRED', "consumedAt"=${now} WHERE "id"=${row.id}`;
      throw validation('EMAIL_VERIFICATION_TOKEN_EXPIRED', 'Verification token has expired.');
    }
    await this.prisma.$transaction(async (tx) => {
      await tx.$executeRaw`UPDATE "CustomerEmailVerification" SET "status"='VERIFIED', "verifiedAt"=${now}, "consumedAt"=${now} WHERE "id"=${row.id}`;
      await tx.customer.update({ where: { id: row.customerId }, data: { email: row.email } });
    });
  ...
  async recordMarketingConsent(customerId, input = {}, context = {}) {
    if (typeof input.isGranted !== 'boolean') throw validation('MARKETING_CONSENT_REQUIRED', 'isGranted must be boolean.');
    const rulesVersion = String(input.rulesVersion || MARKETING_RULES_VERSION);
    if (rulesVersion !== MARKETING_RULES_VERSION) throw validation('MARKETING_RULES_VERSION_STALE', 'Marketing rules version is stale.');
    const now = this.clock();
    const id = crypto.randomUUID();
    const grantedAt = input.isGranted ? now : null;
    const revokedAt = input.isGranted ? null : now;
  ...
  async listNotifications(customerId, { limit = 50 } = {}) {
    const safeLimit = Math.min(Math.max(Number(limit) || 50, 1), 100);
    return this.prisma.$queryRaw`SELECT "id","type","title","body","importance","actionType","actionPayload","significant","createdAt","readAt" FROM "CustomerNotification" WHERE "customerId"=${customerId} ORDER BY "createdAt" DESC LIMIT ${safeLimit}`;
  }

  async markNotificationRead(customerId, notificationId) {
    const now = this.clock();
    const affected = await this.prisma.$executeRaw`UPDATE "CustomerNotification" SET "readAt"=${now} WHERE "id"=${notificationId} AND "customerId"=${customerId} AND "readAt" IS NULL`;
    return { notificationId, readAt: now, changed: Number(affected) > 0 };
  }
