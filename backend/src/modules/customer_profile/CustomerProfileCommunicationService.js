...
class CustomerProfileCommunicationService {
  ...
  async #queueVerifiedEmail(customerId, notificationId, notification) {
    ...
    const verifiedRows = await this.prisma.$queryRaw`SELECT "email" FROM "CustomerEmailVerification" WHERE "customerId"=${customerId} AND "status"='VERIFIED' ORDER BY "verifiedAt" DESC LIMIT 1`;
    if (!verifiedRows?.[0]?.email) return;
    await this.crmRuntime.queueNotification(customerId, { channel: 'EMAIL', subject: notification.title, body: notification.body }, {
      actorId: 'system', authMethod: 'system', correlationId: `customer-notification:${notificationId}`, idempotencyKey: `customer-notification-email:${notificationId}`,
    });
  }
}
