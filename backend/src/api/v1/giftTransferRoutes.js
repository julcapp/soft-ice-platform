...
function createGiftTransferRouter({ authCoreService, giftTransferRuntime }) {
  const router = express.Router(); const auth = createCustomerAuthenticator(authCoreService);
  ...
  router.get('/gifts', auth, asyncHandler(async (req,res)=>sendData(res,req,giftTransferRuntime.listOwn(req.securityContext.subject_id))));
  router.get('/gifts/:id', auth, asyncHandler(async (req,res)=>sendData(res,req,giftTransferRuntime.getOwn(req.securityContext.subject_id,req.params.id))));
...
function createAdminGiftTransferRouter({ giftTransferRuntime }) {
  const router = express.Router(); const auth = createAdminAuthenticator();
  ...
  router.get('/',auth,permit('gift_transfer.admin_view'),(req,res)=>sendData(res,req,giftTransferRuntime.service.repository.list().map(adminGiftDto)));
  router.get('/statistics',auth,permit('gift_transfer.admin_view'),(req,res)=>sendData(res,req,giftTransferRuntime.statistics()));
  router.get('/:id',auth,permit('gift_transfer.admin_view'),(req,res)=>sendData(res,req,adminGiftDto(giftTransferRuntime.service.repository.findById(req.params.id))));
  router.get('/:id/deliveries',auth,permit('notification_delivery.view'),(req,res)=>{ const gift=giftTransferRuntime.service.repository.findById(req.params.id); const rows=[...giftTransferRuntime.service.repository.deliveries.values()].filter((x)=>x.correlationId===gift?.correlationId); sendData(res,req,rows); });
  return router;
}
