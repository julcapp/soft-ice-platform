...
Organization 360 владеет организациями, деревом подразделений, сотрудниками, организационными ролями, точками и назначениями ответственности. Он ссылается на существующий Machine и читает показатели/события из владеющих доменов, не копируя их. Организационные роли не заменяют Auth Core RBAC; tenant scope обязателен, `PLATFORM_OWNER` имеет глобальный доступ, критические изменения аудируются. Недолговечные или отсутствующие проекции честно маркируются `FOUNDATION_ONLY`.
