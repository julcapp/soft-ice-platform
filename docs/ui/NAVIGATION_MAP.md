...
`Клиенты → Customer 360 → Внешние каналы`; `Автоматы → Карточка/Цифровой двойник → Связь и SIM-карта`.
# Видеонаблюдение

`Автоматы → Цифровой двойник → Камера и видеонаблюдение`.
# Центр событий

`Admin Console → Платформа → Центр событий → Событие`

В карточке автомата и Customer 360 используется локальная вкладка/секция «События».
# Подарки

Mini App: профиль → «Мои подарки» → карточка подарка. Предоплаченный заказ → «Отказаться или подарить заказ». Admin Console: «Клиенты» → «Подарки и приглашения».
