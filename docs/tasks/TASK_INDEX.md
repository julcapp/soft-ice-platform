...
| MVP-VS-001 | Customer Registration Flow | MVP Launch Sprint 1 | Implemented | Implemented Telegram Mini App session exchange, Auth Core opaque sessions, Customer Runtime identity resolution, automatic zero-balance Club Account creation, API v1 customer/account/bootstrap endpoints plus `POST /api/auth/telegram` and `GET /api/customer/me` compatibility aliases, Prisma persistence and backend tests without payments, top-ups or machine integration |
| MVP-VS-002 | Club Account and Loyalty Core | MVP Launch Sprint 1 | Implemented | Implemented Club Account status and balance projection, immutable credit/debit ledger records, initial MVP deposit/top-up support, own account/history API endpoints at `/api/v1/club-account/*`, singular/plural route compatibility and backend tests without YooKassa, real payments, machine integration or order fulfillment |
| MVP-VS-003 | Order and Purchase Core | MVP Launch Sprint 1 | Implemented | Implemented Order Runtime, Prisma `OrderStatus` and MVP core order fields, protected order create/read/history API endpoints, internal payment confirmation to `PAID`, `OrderCreated`/`OrderPaid`/`OrderCancelled` event generation and Club Account future integration point without YooKassa, machine dispatch or Telegram notifications |
| MVP-VS-004 | Machine Integration and Dispense Flow | MVP Launch Sprint 1 | Implemented | Implemented Machine Runtime, machine registration/read API, paid-order `DispenseRequest` creation, stored `DispenseCommand`, machine dispense requested/started/completed/failed events, protected order dispense lookup and backend tests without vendor SDKs, Huaxin integration, real telemetry, payment providers or Telegram notifications |
